...
def read_data(dataset, idx, is_train=True):
    # 规范化数据集名称：首字母大写（匹配目录结构）
    dataset_normalized = dataset.capitalize()
    
    # 获取当前文件所在目录，向上两级到项目根目录
    current_dir = os.path.dirname(os.path.abspath(__file__))  # system/utils/
    project_root = os.path.dirname(os.path.dirname(current_dir))  # 项目根目录
    
    if is_train:
        data_dir = os.path.join(project_root, 'dataset', dataset_normalized, 'train')
    else:
        data_dir = os.path.join(project_root, 'dataset', dataset_normalized, 'test')
