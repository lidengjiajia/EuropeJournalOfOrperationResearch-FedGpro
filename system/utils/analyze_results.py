...
# Ablation study configurations (simplified to 8 core configs)
...
def parse_folder_name(folder_name):
    ...
    # 特殊处理中心化结果（格式: Uci_Centralized, Xinwang_Centralized）
    if len(parts) == 2 and parts[1] == 'Centralized':
        dataset = parts[0]
        return ('Centralized', dataset, None)  # 特殊标记
...
def collect_all_results(results_dir):
    """
    收集所有实验结果，分别存储基线实验、消融实验和中心化结果
    
    Args:
        results_dir: system/results目录路径
    
    Returns:
        tuple: (baseline_results, ablation_results, centralized_results)
            ...
            centralized_results: {dataset: [metrics_dict_1, ...]}  # 只使用最佳配置
    """
    baseline_results = defaultdict(list)
    ablation_results = defaultdict(list)
    centralized_results = defaultdict(list)
    ...
    for folder_name in os.listdir(results_dir):
        ...
        # 处理中心化结果（特殊标记）
        if len(parsed) == 3 and parsed[0] == 'Centralized':
            dataset = parsed[1]
            # 读取最佳配置的h5文件（LowLR for Uci, LargeBatch for Xinwang）
            best_config = 'LowLR' if dataset == 'Uci' else 'LargeBatch'
            h5_files = sorted([f for f in os.listdir(folder_path) 
                              if f.endswith('.h5') and best_config in f])
            
            if len(h5_files) == 0:
                # 如果没有最佳配置，使用Default
                h5_files = sorted([f for f in os.listdir(folder_path) 
                                  if f.endswith('.h5') and 'Default' in f])
            
            for h5_file in h5_files:
                h5_path = os.path.join(folder_path, h5_file)
                metrics = read_h5_metrics(h5_path)
                if metrics is not None:
                    centralized_results[dataset].append(metrics)
            
            if centralized_results[dataset]:
                print(f"Centralized: {dataset} | Config: {best_config} | Files: {len(h5_files)}")
            continue
    ...
    return baseline_results, ablation_results, centralized_results
...
def generate_dataset_heterogeneity_tables(baseline_results, centralized_results=None):
    """
    为每个数据集的每种异质性生成单独的表格
    
    Args:
        baseline_results: {(dataset, algorithm, heterogeneity): [metrics_dict_1, ...]}
        centralized_results: {dataset: [metrics_dict_1, ...]} 中心化结果（可选）
    ...
    for dataset in datasets:
        for heterogeneity in heterogeneities:
            ...
            # 首先添加中心化结果（作为性能上界参考）
            if centralized_results and dataset in centralized_results:
                cent_metrics = centralized_results[dataset]
                cent_stats = compute_statistics(cent_metrics)
                if cent_stats is not None:
                    cent_row = {
                        'Algorithm': 'Centralized*',  # 加星号表示上界
                        'Runs': cent_stats.get('accuracy_count', 0)
                    }
                    for metric in ['accuracy', 'precision', 'recall', 'f1']:
                        mean_key = f'{metric}_mean'
                        std_key = f'{metric}_std'
                        if mean_key in cent_stats:
                            mean_val = cent_stats[mean_key]
                            std_val = cent_stats.get(std_key, 0.0)
                            cent_row[metric.capitalize()] = format_mean_std(mean_val, std_val)
                        else:
                            cent_row[metric.capitalize()] = 'N/A'
                    rows.append(cent_row)
...
def plot_ablation_convergence(ablation_results, base_dir, results_dir):
    ...
    Nature style: clean, elegant, no titles
    ...
    config_labels = {
        ...
        'No_VAE_Generation': 'No VAE',
        'Privacy_Epsilon_1.0': 'DP(ε=1)',
        'Privacy_Epsilon_10.0': 'DP(ε=10)',
    }
    
    # Nature 风格柔和配色
    colors = {
        'Full_Model': NATURE_COLORS['blue'],
        'No_Prototype': NATURE_COLORS['purple'],
        'No_VAE_Generation': NATURE_COLORS['orange'],
        'Privacy_Epsilon_1.0': NATURE_COLORS['coral'],
        'Privacy_Epsilon_10.0': NATURE_COLORS['teal'],
    }
    ...
    for dataset in ['Uci', 'Xinwang']:
        ...
        # Plot convergence curves - Nature style
        ...
        for config in target_configs:
            ...
            ax.plot(rounds, mean_acc, 
                    ...
                    linewidth=2.0 if config == 'Full_Model' else 1.5,
                    alpha=0.9)
...
def plot_generalization_ablation(ablation_results, base_dir, results_dir):
    """
    Plot generalization ability ablation study - Nature style
    
    Each dataset plot shows 4 heterogeneity types × 2 configurations = 8 curves
    - Colors: Different heterogeneity types
    - Line styles: Full Model (solid), Reserve 2 (dashed)
    ...
    # Configurations to compare (已简化为2个)
    configs = ['Full_Model', 'Generalization_Reserve_2']
    config_labels = {
        'Full_Model': 'Full (10 clients)',
        'Generalization_Reserve_2': 'Reserve 2 (8 clients)',
    }
    ...
    # Nature 风格柔和配色
    hetero_colors = {
        'feature': NATURE_COLORS['blue'],
        'label': NATURE_COLORS['orange'],
        'quantity': NATURE_COLORS['purple'],
        'iid': NATURE_COLORS['green'],
    }
    ...
    for dataset_name in ['Uci', 'Xinwang']:
        ...
        # Collect and plot data for each heterogeneity type and configuration
        for hetero_type in ['feature', 'label', 'quantity', 'iid']:
            for config in configs:
                key = (dataset_name, config, hetero_type)
                if key not in ablation_results:
                    continue
                
                # Read training history from h5 files
                folder_name = f"{dataset_name}_FedGpro_Ablation_{config}_{hetero_type}"
                ...
                all_accuracies = []
                h5_files = list(folder_path.glob(f"{dataset_name}_*_{hetero_type}_*.h5"))
                ...
                # Generate label
                label = f"{hetero_labels[hetero_type]} - {config_labels[config]}"
                
                # Plot
                ax.plot(rounds, mean_acc, 
                       label=label,
                       color=hetero_colors[hetero_type],
                       linestyle=linestyles[config],
                       linewidth=2.0 if config == 'Full_Model' else 1.5,
                       alpha=0.85)
        
        # Configure plot - Nature style, no title
        ax.set_xlabel('Communication Rounds', fontsize=11)
        ax.set_ylabel('Test Accuracy', fontsize=11)
        # 不设置标题 - Nature 风格
...
def main():
    ...
    # 设置路径 - 从 system/utils 目录回溯到项目根目录
    base_dir = Path(__file__).parent.parent.parent  # system/utils -> system -> project_root
    ...
    baseline_results, ablation_results, centralized_results = collect_all_results(str(results_dir))
    
    print(f"\n[INFO] Baseline configurations: {len(baseline_results)}")
    print(f"[INFO] Ablation configurations: {len(ablation_results)}")
    print(f"[INFO] Centralized datasets: {len(centralized_results)}")
    ...
    baseline_tables = generate_dataset_heterogeneity_tables(baseline_results, centralized_results)
    ...
    # ========== 基线实验Excel文件 ==========
    baseline_excel = summary_dir / 'baseline_experiments.xlsx'
    ...
    # ========== 消融实验Excel文件 ==========
    ablation_excel = summary_dir / 'ablation_experiments.xlsx'
